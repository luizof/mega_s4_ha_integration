"""Backup platform for the MEGA S4 integration."""
...
# S3 multipart upload: minimum 5 MiB per part.
# We use 20 MiB to avoid too many parts. Each part is buffered in memory.
...
def handle_boto_errors(
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        try:
            ...
            error_msg = f"MEGA S4: Failed during {func.__name__}"
...
async def async_get_backup_agents(
    ...
    entries: list[MegaS4ConfigEntry] = hass.config_entries.async_loaded_entries(DOMAIN)
    return [MegaS4BackupAgent(hass, entry) for entry in entries]
...
class MegaS4BackupAgent(BackupAgent):
    """Backup agent for the MEGA S4 integration."""

    domain = DOMAIN

    def __init__(self, hass: HomeAssistant, entry: MegaS4ConfigEntry) -> None:
        """Initialize the MEGA S4 backup agent."""
        super().__init__()
    ...
    async def async_upload_backup(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            raise BackupAgentError("MEGA S4: Failed to upload backup") from err
        else:
